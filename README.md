...
| **v5.4** | **Qwen3.8-27B-NVFP4** | **Dense multimodal** | **27B** | **~24 agentic / ~17 prose** | **Yes** | **Current — no model change. Swaps and rollbacks restore from `/opt/model-archive` instead of re-downloading; the drafter is no longer pruned. Deploy this tag** |
...
cd ~/spark-sovereign
git checkout v5.4   # latest release tag — pins the model and every serving parameter
cp .env.example .env
